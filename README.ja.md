...
各辞書には同じ 4,599 個のソースキーが含まれます。`dict.json` は、このプロジェクトの以前のリリースとの互換性のため、中国語（繁体字）のファイル名として維持されています。
...
このリポジトリでは現在、ベースイメージを `ghcr.io/berriai/litellm:v1.94.0` に固定しています。別の LiteLLM リリースを使用するには、`FROM` 行だけを [`Dockerfile`](Dockerfile) で変更してから、再ビルドして Admin UI を確認してください。
...
現在の辞書は LiteLLM `v1.94.0` に同梱される UI を対象としています。新しい LiteLLM バージョンでは新しい英語文字列が追加されることがあり、すべての辞書にキーを追加するまで、それらの文字列は英語のままです。
