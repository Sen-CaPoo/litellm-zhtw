...
本仓库当前将基础镜像固定为 `ghcr.io/berriai/litellm:v1.99.0`。如需使用其他 LiteLLM 版本，只需修改 [`Dockerfile`](Dockerfile) 中的 `FROM` 行，然后重新构建并检查 Admin UI。
...
当前字典面向 LiteLLM `v1.99.0` 附带的 UI。较新的 LiteLLM 版本可能引入新的英文字符串；在将其键添加到每个字典前，这些字符串会保持英文。
