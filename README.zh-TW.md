...
本儲存庫目前將基底映像檔固定為 `ghcr.io/berriai/litellm:v1.101.0`。若要使用其他 LiteLLM 發行版本，僅需變更 [`Dockerfile`](Dockerfile) 中的 `FROM` 行，然後重新建置並檢查 Admin UI。
...
目前字典的目標為 LiteLLM `v1.101.0` 內附的 UI。新版 LiteLLM 可能引入新的英文文字；在新增至每份字典的鍵之前，這些文字會維持英文。
