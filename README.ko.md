...
각 사전에는 동일한 4,599개의 원본 키가 있습니다. `dict.json`은 이 프로젝트의 이전 릴리스와의
...
이 저장소는 현재 기본 이미지를 `ghcr.io/berriai/litellm:v1.94.0`으로 고정합니다.
...
현재 사전은 LiteLLM `v1.94.0`에 번들된 UI를 대상으로 합니다. 이후 LiteLLM 버전에는 새 영어 문자열이
